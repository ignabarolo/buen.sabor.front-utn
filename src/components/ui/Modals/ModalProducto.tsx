...
const ModalProducto: React.FC<ModalProductoProps> = ({
  ...
  const onDeleteProductoDetalle = async (
    ...
    try {
      if (isEditMode && productoAEditar) {
        ...
        await productoDetalleService.delete(
  ...
  const handleNewIngredient = async () => {
    if (selectedInsumoId !== null && cantidadInsumo > 0) {
      try {
        ...
        const createdDetalle = await productoDetalleService.post(
          `${URL}/ArticuloManufacturadoDetalle`,
          newDetalle
        );
    ...
    },
    {
      ...
      renderCell: (element) => (
        <>
          ...
          {element?.articuloInsumo?.unidadMedida?.denominacion || "N/A"}
        </>
      ),
    },
  ];
  ...
  const handleSubmit = async (values: IArticuloManufacturado) => {
    try {
      ...
      if (isEditMode && productoAEditar) {
        ...
        response = await productoService.put(
          `${URL}/ArticuloManufacturado`,
          productoAEditar.id,
          productoPut
        );
      } else {
        response = await productoService.post(
          `${URL}/ArticuloManufacturado`,
          productoPost
        );
      }
